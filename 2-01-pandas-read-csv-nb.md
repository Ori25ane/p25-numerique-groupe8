...
```{code-cell} ipython3
df[['Age', 'Fare']].describe()
#df dico qui récupere les info selon le nom des colonnes 
```
...
```{code-cell} ipython3
pt = pd.read_csv("data/petit-titanic.csv")
pt.head(2)
# Les colonnes ne sont pas bien séparées, la première ligne est déjà une personne et pas le nom des colonnes
```
...
```{code-cell} ipython3
Titres=["PassengerId",	"Survived","Pclass", "Name","Sex","Age","SibSp","Parch","Ticket","Fare","Cabin","Embarked"]
pt = pd.read_csv("data/petit-titanic.csv", sep=";", header=None, names=Titres)
pt.head(2)
```
...
```{code-cell} ipython3
pt[["Age","Fare"]].describe()
```

4. affichez le nombre de colonnes et de lignes

```{code-cell} ipython3
print("lignes :" ,pt.shape[0] ,"  ;   colonnes :" , pt.shape[1] )
```
