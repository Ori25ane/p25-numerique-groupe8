...
```{code-cell} ipython3
#les colonnes sont "date" "confirmed" "edeaths" et "recovered"
```
...
concatene = pd.concat([dfr,dit], axis = 0)
concatene
```
...
````{admonition} exo
5. généralisez et construisezune dataframe avec tous les pays  
vous aurez sans doute besoin d'utiliser un `for` python
````

```{code-cell} ipython3
for pays in by_country.keys(): 
    dpays = pd.DataFrame(by_country[pays])
    dpays['country'] = pays
    if pays == 'Afghanistan' :
        df = dpays
    else :
        df = pd.concat([df,dpays], axis = 0)
...
```{code-cell} ipython3
dfpivot = pd.pivot_table(df, index=['country','date'], aggfunc='sum')
#aggfunc='sum' est la au cas ou une couple pays,date n'est pas unique

#deuxieme version
clean_df = df.set_index(['country','date'])
...
```{code-cell} ipython3
extrait = pd.concat([clean_df.loc[('France')],clean_df.loc[('Italy')]], axis = 0)
extrait
#on devrait de nouveau rajouter une colonne avec les noms des pays


# on aurait pu utiliser | peutetre
```
...
```{code-cell} ipython3
clean_df.loc[('France')][['deaths','confirmed']]
```
...
```{code-cell} ipython3
df3 = clean_df.loc[('France')][['deaths','confirmed']]

for pays in ('Italy', 'Germany') : 
    df3 = pd.concat([df3, clean_df.loc[(pays)][['deaths','confirmed']]], axis=1)
...
def extract(Lpays,Lmesures,start,finish):
    custom = clean_df.loc[(Lpays[-1])][Lmesures]
    Lpays.pop()
    if Lpays == [] :
        return custom.loc[start : finish]
    for pays in Lpays : 
        custom = pd.concat([custom, clean_df.loc[(pays)][Lmesures]], axis=1)
    return custom.loc[start : finish]

    
extract(['Italy','Germany','France'], ['deaths','confirmed'], '2020-01-22', '2021-09-02') == df3
```

en utilisant cette fonction, plottez sur un même graphique les données de deux pays

```{code-cell} ipython3
extract(['Italy','Germany'], ['deaths','recovered'], '2020-01-22', '2021-07-02').plot() ;
```
...
```{code-cell} ipython3
#données lissées
extract(['Italy','France','Germany'], ['deaths','recovered'], '2020-03-22', '2021-06-02').rolling(window=30).mean().plot() ;
```
...
def cas(Lpays,start,finish) :
    Lpays2 = copy.deepcopy(Lpays)
    
    return np.extract(Lpays,['deaths'],start,finish)+ np.extract(Lpays2,['recovered'],start,finish)

cas(['Italy'], '2020-03-22', '2021-06-02')

#j'ai tout essayé mais je ne comprends pas l'erreur "TypeError: extract() takes 2 positional arguments but 4 were given"
```
...
```{code-cell} ipython3
extract(clean_df.index.get_level_values('country').unique().tolist(), ['deaths','recovered'], '2012-02-22', '2025-07-02').plot() ;
```
...
```{code-cell} ipython3
extract(['France','China'], ['deaths','recovered'], '2013-03-22', '2025-06-02').plot() ;
```
