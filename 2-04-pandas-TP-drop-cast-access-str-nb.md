...
```{code-cell} ipython3
df.info()
#la colonne Size n'a rien de non-null, tout est vide
```
...
df['Mass (lb) orig'] = df['Mass (lb) orig'].str.replace('<', '', regex=False)
df['Mass (lb) orig'] = df['Mass (lb) orig'].str.replace('>', '', regex=False)

df['Mass (lb) orig'] = pd.to_numeric(df['Mass (lb) orig'], errors='coerce').astype('Int64')
#j'avais des NaN 
...
```{code-cell} ipython3
df['Mass (lb) orig'] = (df['Mass (lb) orig']/2.205).astype(float)
df.head()
```
...
```{code-cell} ipython3
total=df.loc['Mass (lb)'].sum
total_US = df.loc[df['Country'] == 'United States', 'Mass (lb)'].sum()
```
...
```{code-cell} ipython3
memorials = df['Artificial object'].str.contains('Memorial', na=False)
df[memorials]
#oui posé par le Luxembourg
```
